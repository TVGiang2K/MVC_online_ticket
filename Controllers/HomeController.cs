...
namespace MVC_online_book_ticket.Controllers
{
    public class HomeController : Controller
    {
        ...
        public async Task<IActionResult> DetailsTrips(int id)
        {
            ...
            var trip = await _context.Trips.Include(x => x.Buses).FirstOrDefaultAsync(x => x.TripsId == id);
            var seat = await _context.Reservations.Include(x => x.Trips).Where(r => r.ActiveReservation == 1).ToListAsync();
            var remain = trip.Buses.SeatCapacity - seat.Count();
            if (trip == null)
                NotFound();
